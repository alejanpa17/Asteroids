...
int main(int argc, char const *argv[]) {
								...
								//Creación de las matrices donde almaceranos las fuerzas de cada asteroide con el resto o con los planetas
								matriz_forces **forces_asteroides = new matriz_forces* [num_asteroids];
								for(int i=0; i<num_asteroids; i++) {
																forces_asteroides[i] = new matriz_forces [num_objects];
								}
								...
								for (int i = 0; i < iterations; i++) {
												...
												#pragma omp parallel for num_threads(2)
																								for (int j = 0; j < num_asteroids; j++) {
																																for (int k = j + 1; k < num_objects; k++) {
																																								...
																																								if (distance > DMIN) {
																																																...
																																																//Almacenamos la fuerza calculada para cada componente al asteroide i
																																																forces_asteroides[j][k].x = force * cos(angles);
																																																forces_asteroides[j][k].y = force * sin(angles);
																																																if (k < num_asteroids) {//Almacenamos la fuerza calculada para cada componente al asteroide j (inversa de la anterior)

																																																								forces_asteroides[k][j].x = -force * cos(angles);
																																																								forces_asteroides[k][j].y = -force * sin(angles);
																																																}
																																								}
																...
																//Sumamos el resultado de las fuerzas entre asteroides y objetos de la matriz
																for(int j=0; j < num_asteroids; j++) {
																								for(int k=0; k < num_objects; k++) {
																																objects[j].force_x += forces_asteroides[j][k].x;
																																objects[j].force_y += forces_asteroides[j][k].y;
																								}
																}
											...
											#pragma omp parallel for num_threads(2)
																...
																for (int j = 0; j < num_asteroids; j++) {
																								...
																								objects[j].force_x = 0;
																								objects[j].force_y = 0;
																								//reiniciamos la matriz de fuerzas también a 0
																								for(int k=0; k < num_objects; k++) {
																																forces_asteroides[j][k].x=0;
																																forces_asteroides[j][k].y=0;
																								}
																}
